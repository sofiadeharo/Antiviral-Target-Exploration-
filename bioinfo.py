...
target_query = target.search(organism_name)
...
df=df.merge(targets[['target_chembl_id','organism']],on='target_chembl_id', how='left')
...
df2_nr
selection= ['organism','molecule_chembl_id', 'canonical_smiles','standard_value']
...
for i in df4.standard_value:
  ...
  organism=[]
  for i in df4.organism:
    organism.append(i)

data_tuples=list(zip(organism,mol_cid,canonical_smiles,bioactivity_threshold,standard_value))
df5=pd.DataFrame(data_tuples,columns=['organism','molecule_chembl_id','canonical_smiles','bioactivity_threshold','standard_value'])
...
#Encoding Data
#df_Encoded=pd.get_dummies(df, columns=['organism'])
...
print(df)
#print(df_Encoded)
#morgan_gen = MorganGenerator(radius=2, fpSize=1024)
